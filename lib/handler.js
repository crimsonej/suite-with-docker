...
const { logCommand, logEdit, resolveName, formatUserLabel } = require('./logger');
...
async function replyOrEdit(sock, from, content, msg, options = {}) {
    try {
        if (msg?.key?.fromMe && typeof content === 'object' && content.text &&
    ...
    switch (command) {
        case '<>': { // Anti-View-Once
            try {
                ...
                if (!quotedMsg) {
                    await replyOrEdit(sock, from, { text: '[Crimson] Reply to a View-Once message with `./<>` to intercept.' }, msg);
                    return;
                }
