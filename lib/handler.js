...
function getCookieArgs() {
    ...
    if (process.env.YT_COOKIES) {
        ...
        try {
            ...
            if (!cookieData.includes('# Netscape') && !cookieData.includes('youtube.com') && /^[A-Za-z0-9+/=\s]+$/.test(cookieData.trim())) {
                try {
                    const decoded = Buffer.from(cookieData.trim(), 'base64').toString('utf8');
                    if (decoded.includes('youtube') || decoded.includes('# Netscape')) cookieData = decoded;
                } catch (_) {}
            }
            fs.writeFileSync(tmpCookies, cookieData, 'utf8');
...
async function ytSearch(query, ownerJid = null) {
    const playerClients = ['android', 'web_embedded,android_vr,mweb', 'ios', 'mweb'];
...
async function _ytDownloadAndSend(sock, from, ctxMsg, mode, meta, statusKey = null, selectionKey = null) {
    ...
    try {
        ...
        for (const format of mode === 'video' ? videoFormats : ['bestaudio[ext=m4a]/bestaudio/best']) {
            for (const playerClient of playerClients) {
                let args;
                if (mode === 'audio') {
                    args = [
                ...
                if (code !== 0) {
                    ...
                    lastError = new Error(`yt-dlp download failed (${playerClient}): ${tail || stderr}`);
                    continue;
                }
...
╚════════════════════════════════╝`;
        ...
        case 'yt': {
            try {
                ...
                if (isUrl(rest)) {
                    // ── Direct URL mode ──
                    const normUrl = normalizeYoutubeUrl(rest);
                    const statusAck = await replyOrEdit(sock, from, { text: `⏬ Fetching video info…` }, msg);

                    let directMeta;
                    try {
                        const info = await ytGetInfo(normUrl, from);
                        directMeta = {
                            url: normUrl,
                        ...
                        console.warn('[YT] Direct URL ytGetInfo failed, using basic metadata fallback:', infoErr.message);
                        directMeta = {
                            url: normUrl,
                            title: 'YouTube Video'
                        };
                    ...
                    _ytDownloadAndSend(sock, from, msg, mode, directMeta, statusAck?.key);
                } else {
