...
const TRANSIENT_ERROR_TTL_MS = 10 * 1000;

function isTransientErrorText(content) {
    return typeof content?.text === 'string' && /^(?:❌|⚠️|🛑|⛔|🚫)/.test(content.text.trim());
}

function removeMessageLater(sock, from, messageKey) {
    if (!messageKey?.id) return;
    const deleteKey = { ...messageKey, remoteJid: messageKey.remoteJid || from, fromMe: true };
    const remove = () => sock.sendMessage(from, { delete: deleteKey }).catch(() => {});
    setTimeout(() => {
        remove();
...
async function replyOrEdit(sock, from, content, msg, options = {}) {
    const transientError = isTransientErrorText(content);
    try {
        const isFromMe = isSenderMe(sock, msg);
        if (isFromMe && typeof content === 'object' && content.text &&
            !content.image && !content.video && !content.document && !content.sticker && !content.audio) {
            const edited = await sock.sendMessageResilient(from, {
                ...content,
                edit: { ...msg.key, remoteJid: from, fromMe: true }
            });
            if (transientError) removeMessageLater(sock, from, { ...msg.key, remoteJid: from, fromMe: true });
            return edited;
        }
    } catch (err) {
        console.log('[REPLY-OR-EDIT] In-place edit failed, falling back:', err.message);
    }
    const sent = await sock.sendMessageResilient(from, content, { quoted: msg, ...options });
    if (transientError) removeMessageLater(sock, from, sent?.key);
    return sent;
}

async function editText(sock, from, key, text) {
    const edited = await sock.sendMessageResilient(from, {
        text,
        edit: { ...key, remoteJid: from, fromMe: true }
    });
    if (isTransientErrorText({ text })) removeMessageLater(sock, from, edited?.key || { ...key, remoteJid: from, fromMe: true });
    return edited;
}
...
function getCookieArgs() {
    const cookiePath = process.env.YT_COOKIES_PATH || path.join(process.cwd(), 'cookies.txt');
    if (fs.existsSync(cookiePath)) {
        return ['--cookies', cookiePath];
    }
    if (process.env.YT_COOKIES) {
        const tmpCookies = path.join(os.tmpdir(), 'yt_env_cookies.txt');
        try {
...
function isYoutubeAuthBlock(text) {
    return /sign in to confirm|please sign in|not a bot|use --cookies/i.test(String(text || ''));
}
...
function cleanPastedCommandText(value) {
    return String(value || '')
    .replace(/\s*#\s*attachment\s*:\s*pasted\s+text\s+#?\d+\s*/gi, ' ')
        .trim();
}
...
function runYtDlpWithProgress(args, timeoutMs, onProgress, ownerJid = null) {
    return new Promise((resolve) => {
        ...
        child.stderr.on('data', (b) => {
            ...
            // --newline makes yt-dlp emit complete progress records, but keep
            // a buffer because stderr chunks can still split records.
            const records = progressBuffer.split(/[\r\n]+/);
            progressBuffer = records.pop() || '';
            for (const record of records) {
                const progressMatch = record.match(/\[download\]\s+([\d.]+)%\s+of\s+~?([\d.]+\w*)\s+at\s+([^\s]+)\s+ETA\s+([^\s]+)/);
                if (!progressMatch || !onProgress) continue;
                try {
                    onProgress({
...
async function ytSearch(query, ownerJid = null) {
    const playerClients = ['tv_embedded', 'ios', 'android', 'web_creator', 'mweb', 'web_embedded', 'web'];
    ...
    for (const playerClient of playerClients) {
        ...
        const { code, stdout, stderr, killed } = await runYtDlp(args, YT_SEARCH_TIMEOUT_MS, ownerJid);
        ...
        if (code === 0 && stdout) {
            ...
            if (parsed.length) {
                return parsed.slice(0, 5).map((j) => ({
                    videoId: j.id || getYoutubeVideoId(j.webpage_url || j.url),
    ...
    for (const playerClient of playerClients) {
        ...
        if (code === 0 && stdout) {
            try {
                ...
                lastError = new Error(`ytGetInfo JSON parse failed: ${e.message}`);
            }
        } else {
            if (isYoutubeAuthBlock(stderr)) {
                throw new Error('YouTube requires cookies on this server. Set YT_COOKIES_PATH or YT_COOKIES in Railway.');
            }
            const tail = stderr.split('\n').slice(-2).join(' | ').slice(0, 300);
            lastError = new Error(`ytGetInfo failed (${playerClient}): ${tail || stderr}`);
        }
    }

    throw lastError || new Error('ytGetInfo failed for all player clients');
}
...
async function _ytDownloadAndSend(sock, from, ctxMsg, mode, meta, statusKey = null, selectionKey = null) {
    ...
    try {
        ...
        // Match the known-good standalone downloader: Android client plus
        // yt-dlp's normal retry/format selection is more reliable than cycling
        // through several clients on a hosted server.
        const playerClients = ['android'];
        const cookieArgs = getCookieArgs();
        const videoFormats = [
            'bestvideo[ext=mp4][height<=720][vcodec^=avc1]/bestvideo[height<=720]+bestaudio[ext=m4a]/best[ext=mp4]/best'
        ];
        let lastError = null;
        ...
        for (const format of mode === 'video' ? videoFormats : ['bestaudio/best']) {
            ...
            if (playerClient === playerClients[0] && !statusKey) {
                try {
                    progressMsg = await replyOrEdit(sock, from, {
                ...
                try {
                    await editText(sock, from, statusKey, `⏬ Starting download (${mode}): *${safeTitle}* …`);
                } catch (err) {
            ...
            if (code !== 0) {
                ...
                console.log(`[YT] Download failed with player_client=${playerClient}: ${tail}`);
            ...
            const candidate = entries
                ...
                .find((e) => /\.(mp3|opus|ogg|m4a|mp4|webm|mkv)$/i.test(e.f));
            ...
            if (downloadedFile || blockedByYoutube) break;
        }
        
        // If we got through all player_clients without success, throw the last error
        if (!downloadedFile && blockedByYoutube) {
            throw new Error('YouTube requires cookies on this server. Set YT_COOKIES_PATH or YT_COOKIES in Railway.');
        }
        ...
        if (!file) {
            if (oversizedBytes) {
                const text = `❌ Video is still too large after reducing quality: ${(oversizedBytes / 1024 / 1024).toFixed(1)} MB > 200 MB cap.`;
                if (statusKey) await editText(sock, from, statusKey, text);
                else await replyOrEdit(sock, from, { text }, ctxMsg);
                return;
            }
        ...
        if (statusKey) {
            try {
                await editText(sock, from, statusKey, `✅ *${safeTitle}* (${mode})`);
            } catch (_) {}
        }
        ...
        if (mode === 'audio') {
            ...
            if (stat.size > MAX_VIDEO_BYTES_DOC) {
                const text = `❌ Video too large: ${(stat.size / 1024 / 1024).toFixed(1)} MB > 200 MB cap.`;
                if (statusKey) await editText(sock, from, statusKey, text);
                else await replyOrEdit(sock, from, { text }, ctxMsg);
                return;
            }
        ...
        try {
            const text = `❌ Download error: ${err.message}`;
            if (statusKey) await editText(sock, from, statusKey, text);
            else await replyOrEdit(sock, from, { text }, ctxMsg);
        } catch (_) {
...
async function handleMessages(sock, msg) {
    ...
    const body = cleanPastedCommandText(msg.message?.conversation || msg.message?.extendedTextMessage?.text || '');
    ...
    if (pickerPending) {
        ...
        if (pickMatch && isQuoted && isReplyToPicker && isFromMe && isFromRequester) {
...
╚════════════════════════════════╝`;
        ...
        case 's': {
            try {
                ...
                for (let i = 0; i < 5; i++) {
                    if (!targetMsg) break;
                    if (targetMsg.ephemeralMessage?.message) { targetMsg = targetMsg.ephemeralMessage.message; continue; }
                    if (targetMsg.viewOnceMessageV2?.message) { targetMsg = targetMsg.viewOnceMessageV2.message; continue; }
                    if (targetMsg.viewOnceMessage?.message) { targetMsg = targetMsg.viewOnceMessage.message; continue; }
                    if (targetMsg.viewOnceMessageV2Extension?.message) { targetMsg = targetMsg.viewOnceMessageV2Extension.message; continue; }
                    if (targetMsg.documentWithCaptionMessage?.message) { targetMsg = targetMsg.documentWithCaptionMessage.message; continue; }
                    break;
                }

                const mediaMsg = targetMsg?.imageMessage || targetMsg?.videoMessage || targetMsg?.stickerMessage || targetMsg?.documentMessage;

                if (!mediaMsg) {
                    await replyOrEdit(sock, from, { text: '❌ Please reply to an image, video, or sticker to convert it to a WebP sticker.' }, msg);
                    return;
                }

                const mime = mediaMsg.mimetype || '';
                const isVideo = mime.startsWith('video/');
                const isImage = mime.startsWith('image/') || mime.includes('webp');
                if (!isVideo && !isImage) {
                ...
                const typeToDownload = isVideo ? 'video' : (mediaMsg.stickerMessage ? 'sticker' : 'image');
                const buffer = await downloadWithRetry(mediaMsg, typeToDownload);

                const tempFile = path.join(os.tmpdir(), `sticker_${Date.now()}_${crypto.randomBytes(4).toString('hex')}.${ext}`);
                const outputFile = `${tempFile}.webp`;
                await fs.writeFile(tempFile, buffer);

                const convertToWebp = (scaleSize = 512, qscale = isVideo ? 60 : 75) => {
                    return new Promise((resolve, reject) => {
                        ...
                        if (isVideo) {
                            cmd.inputOptions(['-ss', '00:00:00', '-t', '6']);
                            cmd.outputOptions([
                                '-vcodec', 'libwebp',
                                '-vf', `scale=${scaleSize}:${scaleSize}:force_original_aspect_ratio=decrease,pad=${scaleSize}:${scaleSize}:(${scaleSize}-iw)/2:(${scaleSize}-ih)/2:color=0x00000000,fps=15`,
                                '-lossless', '0',
                                '-compression_level', '6',
                            ...
                            cmd.outputOptions([
                                '-vcodec', 'libwebp',
                                '-vf', `scale=${scaleSize}:${scaleSize}:force_original_aspect_ratio=decrease,pad=${scaleSize}:${scaleSize}:(${scaleSize}-iw)/2:(${scaleSize}-ih)/2:color=0x00000000`,
                                '-lossless', '0',
                                '-compression_level', '6',
                                '-qscale', String(qscale),
                                '-preset', 'default',
                        ...
                        cmd.toFormat('webp').save(outputFile);
                    });
                };

                try {
                    await convertToWebp(512);
                    let stickerBuffer = await fs.readFile(outputFile);

                    // If larger than WhatsApp's 1MB limit, recompress at 384x384
                    if (stickerBuffer.length > 1000000) {
                        await convertToWebp(384, 45);
                        stickerBuffer = await fs.readFile(outputFile);
                    }
        ...
        case 'delete': {
            ...
            const stickerGroups = settings.autodelete.sticker_groups || (settings.autodelete.sticker_groups = []);
            ...
            if (targetRaw?.toLowerCase() === 'sticker' || args[0]?.toLowerCase() === 'sticker') {
                let turnOn = false;
                if (action === 'on') turnOn = true;
                else if (action === 'off') turnOn = false;
                else turnOn = !stickerGroups.includes(from);

                if (turnOn) {
                    if (!stickerGroups.includes(from)) stickerGroups.push(from);
                    await saveSettings(settings);
                    await replyOrEdit(sock, from, { text: 'No sticker allowed' }, msg);
                } else {
                    const idx = stickerGroups.indexOf(from);
                    if (idx !== -1) stickerGroups.splice(idx, 1);
                    await saveSettings(settings);
                    await replyOrEdit(sock, from, { text: '✅ Sticker auto-delete DISABLED in this group.' }, msg);
                }
                break;
            }
            ...
            if (action === 'list') {
                const listText = groupTargets.length
                    ...
                    : 'No targets set.';
                const stickerActive = stickerGroups.includes(from) ? 'ENABLED' : 'DISABLED';
                await replyOrEdit(sock, from, { text: `🗑 *Auto-Delete Targets*\n\n${listText}\n\n🏷 Sticker Auto-Delete: *${stickerActive}*\n\nOnly works in groups where you are admin.` }, msg);
                break;
            }
            ...
            if (!action || !targetRaw) {
                const stickerStatus = stickerGroups.includes(from) ? 'ENABLED' : 'DISABLED';
                await replyOrEdit(sock, from, {
                    text: '🗑 *Auto-Delete* — instantly removes target messages or stickers (groups, admin required).\n\n' +
                          'Usage:\n' +
                          '• ./delete <number|jid> on|off\n' +
                          '• ./delete on|off (reply to target\'s message)\n' +
                          '• ./delete sticker [on|off] (Auto-delete stickers)\n' +
                          '• ./delete list\n\n' +
                          `🏷 Sticker Auto-Delete in this group: *${stickerStatus}*`
                }, msg);
                break;
            }
        ...
        case 'yt': {
            try {
                ...
                const rest = cleanPastedCommandText(args.slice(1).join(' '));
                ...
                if (isUrl(rest)) {
                    // ── Direct URL mode: resolve through the same search result shape ──
                    const videoId = getYoutubeVideoId(rest);
                    if (!videoId) {
                        await replyOrEdit(sock, from, { text: '❌ Could not read a YouTube video ID from that URL.' }, msg);
                        break;
                    }

                    await replyOrEdit(sock, from, { text: `🔎 Finding the exact video for ID: *${videoId}* …` }, msg);
                    const matches = await ytSearch(videoId, from);
                    const selected = matches.find(result => result.videoId === videoId);
                    if (!selected) {
                        await replyOrEdit(sock, from, { text: '❌ That video was not found in YouTube search results.' }, msg);
                        break;
                    }

                    const directMeta = {
                        ...selected,
                        url: normalizeYoutubeUrl(selected.url)
                    };
                    _ytDownloadAndSend(sock, from, msg, mode, directMeta, msg.key)
                        .catch((e) => {
                    ...
                    await editText(sock, from, searchAck.key, listText);
                    ...
                    const entry = {
                        ...
                        requesterJid: sender,  // Store who requested it
                        ctxMsg: msg,
