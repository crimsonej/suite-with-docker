...
function removeMessageLater(sock, from, messageKey) {
    ...
    setTimeout(() => {
        sock.sendMessage(from, { delete: messageKey }).catch(() => {});
...
async function replyOrEdit(sock, from, content, msg, options = {}) {
    ...
    try {
        ...
        if (isFromMe && typeof content === 'object' && content.text &&
            ...
            if (transientError) removeMessageLater(sock, from, edited?.key || { ...msg.key, remoteJid: from, fromMe: true });
            return edited;
        }
    } catch (err) {
...
function cleanPastedCommandText(value) {
    return String(value || '')
        .replace(/#attachment:\s*pasted\s+text\s+#\d+/gi, '')
        .trim();
}
